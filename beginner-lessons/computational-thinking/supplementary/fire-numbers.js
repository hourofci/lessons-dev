...
function Advance() {
  ...
  for (var x = 1; x < TREE_COUNT_X - 1; x++) {
    ...
    for (var y = 1; y < TREE_COUNT_Y - 1; y++) {
      ...
      tree.innerHTML = tree.getAttribute("data-nextNumber");
...
function CreateTrees() {
  ...
  for (var y = 0; y < TREE_COUNT_Y; y++) {
    ...
    for (var x = 0; x < TREE_COUNT_X; x++) {
      ...
      tree.style.border = "1px solid black";
  ...
  for (var x = 1; x < TREE_COUNT_X - 1; x++) {
    ...
    for (var y = 1; y < TREE_COUNT_Y - 1; y++) {
      ...
      if (tree.innerHTML === "1") {
        ...
        tree.setAttribute("data-nextNumber", "2");
      ...
      // Otherwise, if the tree is not currently burning
        ...
        tree.setAttribute("data-nextNumber", tree.innerHTML);

      } // end else

      // If the tree will be non-burning in the next time step
      if (tree.getAttribute("data-nextNumber") === "0") {
...
function TrySpread(tree) {
  ...
  if (SpreadFromTop(tree) ||
    ...
    tree.setAttribute("data-nextNumber", "1");
